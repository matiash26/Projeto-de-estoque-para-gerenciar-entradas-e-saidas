...
function Statistics() {
  ...
  const gastoTotal = () => {
    return statistic?.expenses?.reduce(
      (acc, el) => (acc += parseFloat(el.gasto)),
      0
    );
  };
    ...
    datasets: [
      {
        ...
        data:
          filter != "day"
            ? statistic?.expensesFilled?.map(({ gasto }) => gasto)
            : null,
      },
      {
        type: "bar",
        ...
        <div className="left-content">
          <div className="top-item">
            <Cards
              ...
              icon={<BiDollar />}
              value={cardsData.ganhos}
              color="green"
            />
            <Cards
              title="Despesas"
              icon={<BiDetail />}
              value={cardsData.gasto}
              color="red"
            />
          </div>
