...
const select = async () => {
    ...
    const sql = "SELECT *, DATE_FORMAT(data, '%d/%m/%Y') as data FROM servicos ORDER BY id DESC;";
    const [row] = await mysql.query(sql)
    return row
}
const find = async (search, status) => {
    const mysql = await client()
    let sql = "SELECT *, DATE_FORMAT(data, '%d/%m/%Y') as data FROM servicos WHERE DATE_FORMAT(data, '%Y-%m-%d') BETWEEN ? AND ? ORDER BY id DESC;"
    const [found] = await mysql.query(sql, [search.start, search.offset])
    return found
}
const insert = async ({ service, gasto }) => {
    const mysql = await client();
    const sql = "INSERT INTO servicos(servico, data, gasto) VALUES(?, ?, ?);";
    const insert = [service, getDate(), +gasto];
    try {
...
const update = async ({ service, gasto, id }) => {
    const mysql = await client();
    const sql = "UPDATE servicos set servico = ?, gasto = ? WHERE id = ?";
    const update = [service, +gasto, id];
    try {
